...
start_test_container() {
  docker run -d --rm -it --name tests-lnurl-withdraw --network=cyphernodeappsnet alpine
  docker network connect cyphernodenet tests-lnurl-withdraw
}

stop_test_container() {
  trace 1 "\n\n[stop_test_container] ${BCyan}Stopping existing containers if they are running...${Color_Off}\n"

  local containers=$(docker ps -q -f "name=tests-lnurl-withdraw")
  if [ -n "${containers}" ]; then
    docker stop $(docker ps -q -f "name=tests-lnurl-withdraw")
  fi
}

exec_in_test_container() {
  docker exec -it tests-lnurl-withdraw "$@" | tr -d '\r\n'
}
...
get_lnurl_withdraw() {
  ...
  local getLnurlWithdraw=$(exec_in_test_container curl -sd "${data}" -H "Content-Type: application/json" lnurl:8000/api)
...
delete_lnurl_withdraw() {
  ...
  local deleteLnurlWithdraw=$(exec_in_test_container curl -sd "${data}" -H "Content-Type: application/json" lnurl:8000/api)
...
force_lnurl_fallback() {
  ...
  local forceFallback=$(exec_in_test_container curl -sd "${data}" -H "Content-Type: application/json" lnurl:8000/api)
  ...
  local currentEpoch=$(exec_in_test_container date -u +"%s")
  trace 3 "[force_lnurl_fallback]   currentEpoch=${currentEpoch}"
  if [ "${expiresAtEpoch}" -ge "${currentEpoch}" ]; then
...
fallback1() {
  ...
  local callbackurlCnWatch1conf="http://${callbackservername}3:${oneconfport}"
  local callbackurl="http://${callbackservername}:${callbackserverport}"

  # Get new address
  local data='{"label":"lnurl_fallback_test"}'
  local btcfallbackaddr=$(exec_in_test_container curl -sd "${data}" -H "Content-Type: application/json" proxy:8888/getnewaddress)
  ...
  local watchresponse=$(exec_in_test_container curl -sd "${data}" -H "Content-Type: application/json" proxy:8888/watch)
...
fallback4() {
  ...
  # 5. Shut down LN02
  # 6. User calls LNServiceWithdraw -> fails because LN02 is down
  # 7. Call ln_pay directly on Cyphernode so that LNURLapp doesn't know about it (simulate delayed LN payment)
  # 8. Call forceFallback -> should check payment status and say it's already paid!
  ...
  local lnpay=$(exec_in_test_container curl -sd "${data}" -H "Content-Type: application/json" proxy:8888/ln_pay)
...
callbackservername="tests-lnurl-withdraw-cb"
